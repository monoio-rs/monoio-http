...
//! details.
//!
//! This could be as basic as using Monoio's [`TcpStream`] to connect to a remote
...
//! ```
//!
//! [`TcpStream`]: https://docs.rs/monoio/latest/monoio/net/tcp/struct.TcpStream.html
...
/// # where T: AsyncReadRent + AsyncWriteRent + Send + Unpin + 'static,
/// # {
...
impl Builder {
    ...
    /// // `client_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let client_fut = Builder::new().max_frame_size(1_000_000).handshake(my_io);
    /// # client_fut.await
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `client_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let client_fut = Builder::new().max_concurrent_streams(1000).handshake(my_io);
    /// # client_fut.await
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `client_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let client_fut = Builder::new().enable_push(false).handshake(my_io);
    /// # client_fut.await
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `client_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let client_fut = Builder::new().handshake::<_, &'static [u8]>(my_io);
    /// # client_fut.await
    /// # }
    /// #
    /// # pub fn main() {}
...
/// # use h2::client::*;
/// #
/// # async fn doc<T: AsyncReadRent + AsyncWriteRent + Unpin>(my_io: T) -> Result<(), h2::Error>
/// # {
/// let (send_request, connection) = client::handshake(my_io).await?;
    ...
    T: AsyncReadRent + AsyncWriteRent + Unpin,
{
    ...
    let (res, _) = io.write_all(msg).await;

    res.map_err(crate::h2::Error::from_io)?;
