...
use monoio_http::{
    common::{body::Body, error::HttpError, request::Request, response::Response},
            ...
            Self::H1(handle) => {
                ...
                match h.next().await {
                    Some(Ok(resp)) => {
                        ...
                        // let frame_read = h.framed_mut();
                        let framed_payload = body_builder.with_io(handle.clone());
                        ...
                        let resp = Response::from_parts(parts, framed_payload);
                        (Ok(resp), false)
                    }
                    Some(Err(e)) => {
                        #[cfg(feature = "logging")]
              ...
              /*     let (resp_fut, send_stream) = match ready_handle.send_request(request,
               * false) {         Ok(ok) => ok,
               *         Err(e) => {
               *             #[cfg(feature = "logging")]
