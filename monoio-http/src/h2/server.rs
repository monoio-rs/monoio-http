...
//! details.
//!
//! This could be as basic as using Monoio's [`TcpListener`] to accept
...
use crate::h2::{
    ...
    frame::{self, Pseudo, PushPromiseHeaderError, Reason, Settings, StreamId},
    proto::{self, Config, Error, Prioritized},
...
/// # use h2::server::*;
/// #
/// # async fn doc<T: AsyncReadRent + AsyncWriteRent + Unpin>(my_io: T) {
...
/// # use h2::server::*;
/// #
/// # fn doc<T: AsyncReadRent + AsyncWriteRent + Unpin>(my_io: T)
/// # -> Handshake<T>
/// # {
    ...
    B: Buf + 'static,
{
    ...
    /// Returns an error if a previous call is still pending acknowledgement
    /// from the remote endpoint.
    pub fn enable_connect_protocol(&mut self) -> Result<(), crate::h2::Error> {
    ...
    /// [`RecvStream`]: ../struct.RecvStream.html
    /// [`SendStream`]: ../struct.SendStream.html
    pub fn poll_closed(&mut self, cx: &mut Context) -> Poll<Result<(), crate::h2::Error>> {
        self.connection.poll(cx).map_err(Into::into)
    }

    #[doc(hidden)]
    #[deprecated(note = "renamed to poll_closed")]
    pub fn poll_close(&mut self, cx: &mut Context) -> Poll<Result<(), crate::h2::Error>> {
        self.poll_closed(cx)
    }
...
impl Builder {
    ...
    /// // `server_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let server_fut = Builder::new().max_frame_size(1_000_000).handshake(my_io);
    /// # server_fut
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `server_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let server_fut = Builder::new().max_concurrent_streams(1000).handshake(my_io);
    /// # server_fut
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `server_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let server_fut = Builder::new().handshake(my_io);
    /// # server_fut
    /// # }
    /// #
    /// # pub fn main() {}
    ...
    /// // `server_fut` is a future representing the completion of the HTTP/2
    /// // handshake.
    /// let server_fut: Handshake<_, &'static [u8]> = Builder::new().handshake(my_io);
    /// # server_fut
    /// # }
    /// #
    /// # pub fn main() {}
...
impl<B: Buf> SendResponse<B> {
    ...
    pub fn send_response(
        ...
        response: Response<()>,
        end_of_stream: bool,
    ) -> Result<SendStream<B>, crate::h2::Error> {
        self.inner
    ...
    pub fn push_request(
        &mut self,
        request: Request<()>,
    ) -> Result<SendPushedResponse<B>, crate::h2::Error> {
        self.inner
    ...
    /// a user error.
    pub fn poll_reset(&mut self, cx: &mut Context) -> Poll<Result<Reason, crate::h2::Error>> {
    ...
    B: Buf + 'static,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let buf: [u8; PREFACE.len()] = [0; PREFACE.len()];
        ...
        {
            let (result, ret_buf) = ready!(self.read_fut.poll(cx));
            let n = result.map_err(crate::h2::Error::from_io)?;

            if n != PREFACE.len() {
                println!("!!!!!!!!!!! ReadPreface failed");
                return Poll::Ready(Err(crate::h2::Error::from_io(io::Error::new(
...
where
    T: AsyncReadRent + AsyncWriteRent + fmt::Debug,
    B: fmt::Debug + Buf,
{
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
