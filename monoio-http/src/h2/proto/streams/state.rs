...
use crate::h2::{
    codec::UserError,
    frame::{self, Reason, StreamId},
    proto::{self, Error, Initiator, PollReset},
};
...
enum Cause {
    ...
    /// - Header block size was too large, so we want to REFUSE, possibly after sending a 431
    ///   response frame.
...
impl State {
    ...
    pub fn recv_reset(&mut self, frame: frame::Reset, queued: bool) {
        match self.inner {
            ...
            // the following cases:
            //
            // - if the cause is `Cause::Scheduled(..)` (i.e. we have not actually closed the stream
            //   yet).
            // - if the cause is `Cause::EndStream`: we transition to this state when an EOS frame
            //   is *enqueued* (so that it's invalid to enqueue more frames), not when the EOS frame
            //   is *sent*; therefore, there may still be frames ahead of the EOS frame in the send
            //   queue.
            //
